export class DefaultPage {
    ...
    openContactForm(
        ...
        cy.contains('label', email_field_title).should('be.visible')
        cy.get('#recipient-email')
            .should('be.visible')
            .and('have.attr', 'type', 'text')
        // check modal name field title and input
        cy.contains('label', name_field_title).should('be.visible')
        cy.get('#recipient-name')
            .should('be.visible')
            .and('have.attr', 'type', 'text')
        // check modal message field title and input
        cy.contains('label', message_field_title).should('be.visible')
    ...
    sendMessage(send_button, success_message) {
        ...
        cy.on('window:alert', cy.stub().returns(true).as('alert'))
        cy.get('@alert')
